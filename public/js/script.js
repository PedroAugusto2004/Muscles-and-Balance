...
function getWorkoutPlan() {
  ...
  setTimeout(() => {
    ...
    if (fitnessLevel === 'beginner') {
      if (goal === 'weight-loss') {
        if (preference === 'cardio') {
          ...
          description =
            'Low-impact cardio and bodyweight exercises for weight loss.';
          exercises = [
          ...
          frequency = '3-4 times per week';
          description =
            'Introductory full-body routine for weight loss and general fitness.';
          exercises = [
        ...
        description =
          'Basic strength exercises for muscle toning and initial gains.';
        exercises = [
          ex('Push-Ups', exerciseVideos['Push-Ups']),
          ex('Bodyweight Squats', exerciseVideos['Bodyweight Squats']),
        ...
        frequency = '3-4 times per week';
        description =
          'Steady, moderate-intensity exercises to improve cardiovascular endurance.';
        exercises = [
    ...
    if (age) {
      if (parseInt(age) < 18)
        personalizedNote +=
          'Note: As a teen, focus on form and avoid heavy weights. ';
      else if (parseInt(age) > 55)
        personalizedNote +=
          'Note: For 55+, prioritize joint-friendly movements and flexibility. ';
    }
    if (gender) {
      if (gender === 'female')
        personalizedNote +=
          'This plan includes glute and core focus, as many women prefer these areas. ';
      if (gender === 'male')
        personalizedNote +=
          'This plan includes chest and arm focus, as many men prefer these areas. ';
    }
    if (equipment) {
      if (equipment === 'none')
        personalizedNote += 'All exercises are bodyweight only. ';
      else if (equipment === 'basic')
        personalizedNote += 'Includes dumbbells and resistance band options. ';
      else if (equipment === 'full-gym')
        personalizedNote +=
          'Includes barbell, machines, and advanced equipment. ';
    }
    if (days) {
    ...
    for (let d = 1; d <= (parseInt(days) || 3); d++) {
      ...
      if (splitType === 'push-pull-legs') {
        if (d % 3 === 1) {
          ...
          dayExercises = pickRandom(exercisePool.upper, 4);
        } else if (d % 3 === 2) {
          focus = 'Pull (Back/Biceps)';
          dayExercises = pickRandom(exercisePool.upper, 4);
        } else {
          focus = 'Legs/Core';
          dayExercises = pickRandom(exercisePool.lower, 3).concat(
            pickRandom(exercisePool.core, 2)
          );
        }
        ...
        // Full body or mixed
        dayExercises = pickRandom(exercisePool.upper, 2).concat(
          pickRandom(exercisePool.lower, 2),
          pickRandom(exercisePool.core, 1)
        );
        focus = 'Full Body';
      }
      // Add a cardio or yoga finisher if user prefers
      if (preference === 'cardio')
        dayExercises.push(pickRandom(exercisePool.cardio, 1)[0]);
      if (preference === 'yoga')
        dayExercises.push(pickRandom(exercisePool.yoga, 1)[0]);
    ...
    document.getElementById('plan-duration').innerText =
      `Duration: ${duration}`;
    document.getElementById('plan-frequency').innerText =
      `Frequency: ${frequency}`;
    document.getElementById('plan-description').innerText =
      personalizedNote + description;
...
function share(platform) {
  ...
  const message = encodeURIComponent(
    'Check the best healthy platform of all time!'
  );
...
button.addEventListener('click', () => {
  ...
  textElement.textContent = hiddenContent.classList.contains('show')
    ? 'Show less'
    : 'Show more';
